        ...
        try:
            ...
            if len(df_daily) >= 10:
                ...
                for i in range(10):
                    ...
                    if pd.notna(current_sma1000):
                        if row['low'] <= current_sma1000 * 1.05 and row['high'] >= current_sma1000 * 0.95:
                            touched_1k = True
                            days_ago = i
                            break
...
def fetch_dashboard_cache(conn) -> pd.DataFrame:
    ...
    if not df.empty:
        ...
        df = df.sort_values(['_sort', 'Ticker']).drop(columns=['_sort']).reset_index(drop=True)
...
def calculate_forming_ha_colors(df: pd.DataFrame, timeframe: str) -> pd.DataFrame:
    ...
    if df_tf.empty:
        return pd.DataFrame(index=df.index, columns=['Prev_HA_Color', 'Forming_HA_Color'])
    ...
    df_tf_prev['Current_HA_Open'] = (df_tf_prev['Prev_HA_Open'] + df_tf_prev['Prev_HA_Close']) / 2
    
    df_tf_prev = df_tf_prev.drop_duplicates(subset=['Period'])
    
    # Merge back to daily
    index_name = df_eval.index.name or 'date'
    df_eval = df_eval.reset_index().merge(df_tf_prev[['Period', 'Prev_HA_Color', 'Current_HA_Open']], on='Period', how='left').set_index(index_name)
    ...
    df_eval['Forming_HA_Close'] = (df_eval['TF_Open'] + df_eval['TF_High'] + df_eval['TF_Low'] + df_eval['close']) / 4
    df_eval['Forming_HA_Color'] = np.where(df_eval['Forming_HA_Close'] >= df_eval['Current_HA_Open'], 'Green', 'Red')
...
def run_backtest(conn, div_pct: float = 2.0) -> dict:
    ...
    for ticker in tickers:
        try:
            ...
            daily_momentum_ok = (price > sma200) & (ema21 > sma200) & (sma50 > sma200) & (sma100 > sma200)
            
            ha_1m = calculate_forming_ha_colors(df, 'ME')
            
            condition = (ha_1m['Prev_HA_Color'] == 'Red') & (ha_1m['Forming_HA_Color'] == 'Green') & daily_momentum_ok
    ...
    if df_trades.empty:
        return {'trades': df_trades, 'summary': pd.DataFrame(), 'avg_trades_per_year': 0, 'avg_trades_per_week': 0}
    ...
    df_trades['Week'] = pd.to_datetime(df_trades['Entry Date']).dt.to_period('W').astype(str)
    avg_trades_per_week = df_trades.groupby('Week').size().mean()
        
    summary = df_trades.groupby('Ticker').agg(
        Trades=('Ticker', 'count'),
    ...
    win_rates = df_trades.groupby('Ticker').agg(
        No_Divs_Win_Rate=('No_Divs_Win', 'mean'),
        DRIP_Win_Rate=('DRIP_Win', 'mean')
    ).reset_index()
    ...
    summary['No_Divs Current Value ($)'] = summary['Invested ($)'] * (1 + summary['No_Divs_Avg_Return_Pct'] / 100)
    summary['DRIP Current Value ($)'] = summary['Invested ($)'] * (1 + summary['DRIP_Avg_Return_Pct'] / 100)
